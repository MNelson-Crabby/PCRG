...
######### This section just for dealing with 2019 Tulalip Data sheet ############################
######### run this alone and not with other tribes data ###################################

####count data
# reading in Tulalip count data
df <- read_xlsx("PCRG raw datasheets/Funky format/QAQC2019LTData_Tulalip.xlsx", sheet = 1, skip = 2)
#creating organization column
df$Organization <- "Tulalip"

#turning date to date object
df$date <- as.Date(df$date)

# making sure a couple columns are numeric
df$`Metacarcinus magister megalopae` <- as.numeric(df$`Metacarcinus magister megalopae`)
df$`Metacarcinus magister instar` <- as.numeric(df$`Metacarcinus magister instar`)
# total dungeness crab (megalope + instars)
df$totalCMagister <- df$`Metacarcinus magister megalopae` + df$`Metacarcinus magister instar`

# dungeness crab CPUE
df$`Hours Fished` <- as.numeric(df$`Hours Fished`)
df$TotalDungenessCPUE <- df$totalCMagister/df$`Hours Fished`
df$DungMegalopeCPUE <- df$`Metacarcinus magister megalopae`/df$`Hours Fished`
df$DungInstarCPUE <- df$`Metacarcinus magister instar`/df$`Hours Fished`

colnames(df)[colnames(df) == "Location (Abrevation)"] <- "Site"

# #read in tideMerge function below
# #adding tidal data
# tidalRange <- read.csv("other input/dailyTidalRange2019_Seattle.csv")
# #converting date column
# tidalRange$date <- as.Date(tidalRange$date, "%m/%d/%Y")
# 
# #merge df with tide data
# countsWithTides<- tideMerge(df,tidalRange)
####
...
#############################################################################################
#Adding Tulalip data into big list

counts_split[[9]] <- df
names(counts_split)[9]<- "Tulalip"
...
############### For single 2019 Tulalip plot output only ##################
# plot <- lightTrapTideGraph_scaler(df, countsWithTides)
# 
# ggsave(paste("output/TULALIP_ONLY_Light_Traps_2019_w_Tides", Sys.Date(), ".pdf", sep=""), 
#        gridExtra::marrangeGrob(grobs = list(plot), nrow=4, ncol=1, top=NULL), width=8.5, height=11)
#####################################################################